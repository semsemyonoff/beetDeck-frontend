...
- `src/lib/` — pure helpers (no React): `route`, `albums`, `library`, `disc`, `diff`, `scan`, `tagEditor`, `platform`, `lyricsFetchQueue`, `useModalDismiss`.
- `src/ui/` — shared widgets (RouteLink, Topbar, Icon, Segmented, Cover, IdentifyModal, useTagRows, FolderTree, TagTable, BulkBar, UntaggedGroup, ItemsIdentifyModal, TagEditorModal, AlbumLyricsModal).
