...
const ITEMS = [
  { id: 1, title: 'Track One', artist: 'Artist A', album: '', track: 1, album_id: 10, path: '/Music/Folder/01.mp3' },
  { id: 2, title: 'Track Two', artist: 'Artist A', album: '', track: 2, album_id: 10, path: '/Music/Folder/02.mp3' },
  { id: 3, title: 'Other Track', artist: 'Artist B', album: '', track: 1, album_id: 20, path: '/Other/Dir/01.mp3' },
];
...
function makeFetch(items = ITEMS, batchResult = { status: 'ok', warnings: [] }) {
  return vi.fn().mockImplementation((url) => {
    if (url === '/api/items/untagged') {
      return Promise.resolve({ ok: true, json: () => Promise.resolve(items) });
    }
    if (url === '/api/items/metadata-batch') {
      return Promise.resolve({ ok: true, json: () => Promise.resolve(batchResult) });
    }
...
describe('Untagged — folder editor', () => {
  ...
  it('album-only bulk edit + Save POSTs all row ids with album fields', async () => {
    ...
    await waitFor(() => {
      ...
      expect(body.items.map((i) => i.id)).toEqual(expect.arrayContaining([1, 2]));
  ...
  it('identify button is disabled until both album and album-artist are set', async () => {
    ...
    const identifyBtn = screen.getByRole('button', { name: /identify via musicbrainz/i });
  ...
  it('shows a flash warning when the batch endpoint returns warnings', async () => {
    vi.stubGlobal(
      'fetch',
      makeFetch(ITEMS, { status: 'ok', warnings: ['file write failed for id 1'] })
    );
    await act(async () => {
      render(<Untagged dir={DIR} />);
    });
